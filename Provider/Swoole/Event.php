...
class Event{
    ...
    /**
     * @var swoole_websocket_server Swoole WebSocket Server
     */
    protected $server;
    ...
    /**
     * @var int WebSocket连接ID
     */
    protected $fd;

    /**
     * @var string WebSocekt服务端本地IP
     */
    protected $host;
    ...
    public function broadcast($data, $groupID = '') {
        ...
        // 通知其它集群节点
        $nodeClients = $this->app->initNodes();
        ...
        foreach ($nodeClients as $key => $nodeClient) {
            ...
            $notifyData->action = 'clusterNotify';
    ...
    public function broadcastself($data, $groupID = '') {
        ...
        $this->debug('broadcastself - node_fds', $this->app->nodeFDs);
        foreach ($connections as $fd) {
            if(isset($this->app->nodeFDs[$fd])){
